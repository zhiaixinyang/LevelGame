 ...
 */
public class PercentageHProgressBarView  extends ProgressBar {
    protected int reachColor = ContextCompat.getColor(App.getAppContext(), R.color.blue_700);
    protected int unReachColor = ContextCompat.getColor(App.getAppContext(), R.color.blue_300);
    protected int reachHeight = DensityUtil.dip2px(getContext(),4);
    protected int unReachHeight = DensityUtil.dip2px(getContext(),3);

    protected int textSize = DensityUtil.dip2px(getContext(),12);
    protected int textColor = ContextCompat.getColor(App.getAppContext(), R.color.black);
    protected int textOffSet = DensityUtil.dip2px(getContext(),10);

    protected Paint paint = new Paint();
    protected int realWidth;

    public PercentageHProgressBarView(Context context) {
        this(context, null);
    }

    public PercentageHProgressBarView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PercentageHProgressBarView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PercentageHProgressBarView);

        reachColor = typedArray.getColor(
                R.styleable.PercentageHProgressBarView_myprogress_reach_color, reachColor);
        unReachColor = typedArray.getColor(
                R.styleable.PercentageHProgressBarView_myprogress_unreach_color, unReachColor);
        reachHeight = (int) typedArray.getDimension(
                R.styleable.PercentageHProgressBarView_myprogress_reach_height, reachHeight);
        unReachHeight = (int) typedArray.getDimension(
                R.styleable.PercentageHProgressBarView_myprogress_unreach_height, unReachHeight);

        textColor = typedArray.getColor(
                R.styleable.PercentageHProgressBarView_myprogress_text_color, textColor);
        textSize = (int) typedArray.getDimension(
                R.styleable.PercentageHProgressBarView_myprogress_text_size, textSize);
        textOffSet = (int) typedArray.getDimension(
                R.styleable.PercentageHProgressBarView_myprogress_text_offset, textOffSet);
