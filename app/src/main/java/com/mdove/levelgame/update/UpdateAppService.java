...
public class UpdateAppService extends Service {
    public static final String EXTRA_UPDATE_APP_URL = "extra_update_app_url";

    private DownloadManager manager;
    private DownloadCompleteReceiver receiver;
    private String url;
    private String DOWNLOADPATH = "/passwordguard/apk/";

    public static void start(Context context, String appUrl) {
        Intent toStart = new Intent(context, UpdateAppService.class);
        toStart.putExtra(EXTRA_UPDATE_APP_URL, appUrl);
        context.startService(toStart);
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        url = intent.getStringExtra(EXTRA_UPDATE_APP_URL);
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
                ...
                if (manager.getUriForDownloadedFile(downId) != null) {
                    ToastHelper.shortToast(context.getString(R.string.string_update_success));

                    installAPK(context, getRealFilePath(context, manager.getUriForDownloadedFile(downId)));
                } else {
                    ToastHelper.shortToast(context.getString(R.string.string_update_err));
                }
